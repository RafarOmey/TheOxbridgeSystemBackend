    ...
    Auth.Authorize(req, res, "admin", function (err) {
        ...
        User.find({ emailUsername: req.body.emailUsername }, function (err, users) {
            ...
            user.save(function (err) {
                ...
                var token = jwt.sign({ id: user.emailUsername, role: "admin" }, config.secret, { expiresIn: 86400 });
...
exports.register = (req, res) => {
    ...
    User.findOne({ emailUsername: req.body.emailUsername }, function (err, user) {
        ...
        user.save(function (err) {
            ...
            // returning a token
            var token = jwt.sign({ id: user.emailUsername, role: "user" }, config.secret, { expiresIn: 86400 });
            res.status(201).send({ auth: true, token: token });
        });
    });
};
...
exports.login = (req, res) => {
    ...
    User.findOne({ emailUsername: req.body.emailUsername }, function (err, user) {
        ...
        var token = jwt.sign({ id: user.emailUsername, role: user.role }, config.secret, { expiresIn: 86400 });
