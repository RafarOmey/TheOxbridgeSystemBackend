...
//         if (!ship)
//             return res.status(404).send({ message: "Ship with id " + registration.shipId + " was not found" });

//         // Checking if event exists
//         const event:IEvent = await Event.findOne({ eventId: registration.eventId });
//             if (!event)
//                 return res.status(404).send({ message: "Race with id " + registration.eventId + " was not found" });
